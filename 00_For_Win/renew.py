#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ClawBot 会话续期守护（renew.py）—— Windows 版
...
v2 改为**只读桌面端的轮询游标文件**，零网络请求、零抢消息：
    ...
    <桌面端数据目录>/claw-state/weixin/<bot_id>.cursor.json
...
★ 与 macOS 版的差异：
  1. **游标目录不再硬编码** —— mac 版写死 `~/.workbuddy/claw-state/weixin`；
     Windows 版改为遍历 `winenv.claw_state_dirs()` 的候选目录，
     命中哪个用哪个，并把「找过哪些路径」写进 status 输出，便于排障。
  2. 失效提示里的命令按平台生成（用**当前解释器的绝对路径**，不写死，见 `winenv.default_python()`）。
  3. `cursor_dir` 可在 `renew_config.json` 里手动指定（覆盖自动探测）。
...
自测（Windows 下把 `python` 换成 `py -3` 亦可）：
  python renew.py            # 执行一次检查（纯本地，无网络请求）
  python renew.py --force    # 强制执行（忽略节流）
  python renew.py status     # 只看状态
  python renew.py where      # 只列出游标目录候选与命中情况（排障用）
  python renew.py reset      # 把计时基线重置为现在
"""
...
import winenv    # noqa: E402  跨平台适配层（游标目录候选 / 解释器）
...
DEFAULTS: dict = {
    ...
    "_help_字段": {
        ...
        "cursor_dir": "游标目录。留空 = 自动探测（见 `python renew.py where`）；探测不到时在这里填绝对路径",
    },
}
...
# 游标新鲜度（核心判据，纯本地）
# ---------------------------------------------------------------------------
def _cursor_dirs() -> list[pathlib.Path]:
    """候选游标目录（★ Windows 差异：不再硬编码单一目录）。"""
    manual = str(load_config().get("cursor_dir") or "").strip()
    dirs: list[pathlib.Path] = [pathlib.Path(manual)] if manual else []
    dirs += winenv.claw_state_dirs()
    out: list[pathlib.Path] = []
    seen = set()
    for d in dirs:
        k = str(d).lower() if winenv.IS_WIN else str(d)
        if k not in seen:
            seen.add(k)
            out.append(d)
    return out
...
def _cursor_name() -> str:
    """按当前 bot_id 推导游标文件名（desktop 端把 "@" 写成 "_"）。"""
    try:
        cid = (clawbot.load_channel() or {}).get("channel_id") or ""
    except Exception:  # noqa: BLE001
        cid = ""
    if not cid:
        return ""
    # example-current-bot@im.bot → example-current-bot_im.bot.cursor.json
    return str(cid).replace("@", "_") + ".cursor.json"


def _cursor_file() -> tuple[pathlib.Path | None, bool]:
    """定位桌面端游标文件。返回 (路径, 是否为模糊匹配)。

    两级策略（★ Windows 差异）：
      ① **精确匹配** —— 用 bot_id 推导文件名，在候选目录里依次找；
      ② **模糊兜底** —— 精确名找不到时，取候选目录中 mtime 最新的 `*.cursor.json`。
         为什么需要：本机无法预知另一台 Windows 客户端把 bot_id 转成文件名的确切规则，
         若规则不同，精确匹配会一直落空 → 监测能力彻底失效（而不是报错，属于静默失效）。
         模糊匹配至少能守住「桌面端有没有在轮询」这个核心判据，并在 status 里标注 guessed。
    """
    dirs = _cursor_dirs()
    name = _cursor_name()
    if name:
        for d in dirs:
            try:
                ...
                continue
    newest: tuple[pathlib.Path, float] | None = None
    for d in dirs:
        try:
            for p in d.glob("*.cursor.json"):
                m = p.stat().st_mtime
...
def cursor_status(now_ts: float | None = None) -> dict:
    """返回 {'found','path','age_hours','mtime_ts','guessed','dirs','bot_id','cursor_name'}。"""
    now_ts = now_ts or datetime.datetime.now().timestamp()
    out: dict = {"found": False, "path": None, "age_hours": None, "mtime_ts": None,
                 "guessed": False, "dirs": [str(d) for d in _cursor_dirs()],
                 "cursor_name": _cursor_name()}
    p, guessed = _cursor_file()
    if not p:
        return out
    out["path"] = str(p)
    out["guessed"] = guessed
...
def _notify_expired(now: datetime.datetime) -> bool:
    # ★ Windows 差异：命令与指引按平台生成（不再硬编码 mac 的解释器绝对路径）
    if winenv.IS_WIN:
        howto = "按下面的方式二选一："
        manual = '打开本文件夹，双击 login.cmd'
    else:
        howto = "在终端执行："
        manual = '"{}" clawbot.py login'.format(winenv.default_python())
    body = "\n".join([
...
if __name__ == "__main__":
    ...
    if action == "status":
        ...
        print(json.dumps({
            ...
            "cursor": {
                ...
                # ★ Windows 差异新增：命中方式与搜索范围，便于在陌生机器上排障
                "expected_file_name": cs.get("cursor_name") or "(未能从 bot_id 推导)",
                "matched_by": ("模糊匹配（未用 bot_id 精确命中，请核对下面的搜索目录）"
                               if cs.get("guessed") else
                               ("bot_id 精确匹配" if cs.get("found") else "(未找到)")),
                "searched_dirs": winenv.describe_paths(list(_cursor_dirs())),
            },
    ...
    if action == "where":
        # ★ Windows 差异新增：列出所有候选目录及其命中情况（陌生机器上用它定位游标）
        cs = cursor_status()
        print(json.dumps({
            "platform": sys.platform,
            "expected_file_name": cs.get("cursor_name") or "(未能从 bot_id 推导)",
            "hit": cs.get("path"),
            "matched_by": "模糊匹配" if cs.get("guessed") else (
                "精确匹配" if cs.get("found") else "(未找到)"),
            "searched_dirs": winenv.describe_paths(list(_cursor_dirs())),
            "hint": "若全部 not exists：说明桌面端还没绑定 ClawBot，或数据目录在别处。"
                    "确认实际位置后填进 renew_config.json 的 cursor_dir 即可。",
        }, ensure_ascii=False, indent=2))
        sys.exit(0)
