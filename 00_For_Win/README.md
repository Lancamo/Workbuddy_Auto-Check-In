...
### 为什么会有第二个计划任务（watchdog）

`catchup.py` 的**所有告警**都建立在「它自己跑起来了」这个前提上。如果计划任务被停用/删除、
Python 被卸载、文件夹被移走 —— 主脚本会**连告警机制一起静默死掉**，你不会有任何感知。
这是整套系统唯一的、无法靠主脚本自己解决的盲区。

所以 `install.py` 会注册**两个**独立任务：

| 任务名 | 间隔 | 干什么 |
|---|---|---|
| `WorkBuddyRewardCatchup` | 5 分钟 | 真正干活的主脚本 |
| `WorkBuddyRewardWatchdog` | 30 分钟 | 只查 `state.json` 心跳（阈值 90 分钟）与主任务是否还在，异常弹 Toast |
...
**如果哪天发现差异行数暴涨**（比如 `catchup.py` 从 149 行涨到 300 行），说明逻辑开始分叉了，
应当回头把公共部分抽出来，而不是继续各改各的。

> 参考基线（2026-09-18 复测）：`catchup.py` 149、`notify.py` 193、`renew.py` 149、
> `clawbot.py` 86、`scripts/api_discovery.py` 54、`scripts/http_client.py` 17。
> **口径必须一起写**，就是上面 5.3 节 ② 那条命令（`diff | grep -c '^[<>]'`）。
> 同一组文件换个口径能差出好几成 —— 用 `diff | wc -l` 数出来是 209 / 258 / 179。
...
自检脚本在 macOS 上跑 Windows 代码路径，**195 项全部通过**（项数随环境浮动，判据是 0 失败）。它实际验证了：

- 计划任务 XML 结构合法（命名空间、触发器、重复间隔、两个关键布尔值、UTF-16 编码往返）
...
- **watchdog 的独立性**（第 8d 节）：用 `ast` 查它真实的 `import` 语句，断言不依赖任何项目模块；
  并注入 `job_loaded` 覆盖「心跳新鲜 / 心跳过期 / state 缺失 / 任务未注册 / 两者并存 / 未超阈值」六种结论
