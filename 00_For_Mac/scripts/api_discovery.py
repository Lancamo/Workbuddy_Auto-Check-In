...
权威来源（本模块读什么）：
  - /Applications/WorkBuddy.app/Contents/Info.plist            → 客户端版本
  - /Applications/WorkBuddy.app/Contents/Resources/app.asar     → 打包的前端源码
...
CLIENT_APP = "/Applications/WorkBuddy.app"
ASAR = CLIENT_APP + "/Contents/Resources/app.asar"
PLIST = CLIENT_APP + "/Contents/Info.plist"
...
def client_fingerprint() -> dict:
    ...
    因为这正是"接口可能换了"的时刻。
    """
    fp = {"app": CLIENT_APP, "path": ASAR, "version": None,
          "asar_size": None, "asar_mtime": None, "available": False}
    try:
        p = pathlib.Path(PLIST)
        if p.exists():
            with p.open("rb") as f:
                info = plistlib.load(f)
            fp["version"] = (info.get("CFBundleShortVersionString")
                             or info.get("CFBundleVersion"))
        a = pathlib.Path(ASAR)
        if a.exists():
            st = a.stat()
            fp["asar_size"] = st.st_size
            fp["asar_mtime"] = int(st.st_mtime)
            fp["available"] = True
    except Exception:  # noqa: BLE001 — 指纹失败不致命，走兜底
        pass
    return fp
